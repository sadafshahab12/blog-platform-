...
const CreateBlog = ({ buttonRef }: CreateBlogProps) => {
  ...
  const editBtnRef = useRef<HTMLHeadingElement | null>(null);
  ...
  const handleUpdatePost = () => {
    setPosts((prevPosts) =>
      prevPosts.map((post) =>
        post.id === editingPostId ? { ...post, title, content, image } : post
      )
    );
    setTitle("");
    setContent("");
  ...
  return (
    <section className="max-w-4xl mx-auto p-8" ref={buttonRef}>
      <h2 className="text-3xl font-bold mb-4" ref={editBtnRef}>
        Create a New Post
      </h2>
      {!userId && (
      ...
      <form
        onSubmit={editingPostId ? handleUpdatePost : handleSubmit}
        className="space-y-6"
      >
        <input
          type="text"
            ...
            <li key={post.id} className="border p-4 mt-4 relative">
                    ...
                    {dropdownOpen === post.id && (
                      <div className="absolute right-0 mt-2 w-32 bg-white border rounded shadow-md">
                        <button
                          ...
                          className="flex items-center gap-3 w-full px-4 py-2 text-left text-sm text-gray-700 hover:bg-gray-200"
                        >
                          <MdEditNote className="w-6 h-6" /> Edit
                        </button>
                        <button
                          onClick={() => handleDeletePost(post.id)}
                          className="flex items-center gap-3 w-full px-4 py-2 text-left text-red-600 hover:bg-gray-200 text-sm"
                        >
              ...
              <div className="mt-4 flex justify-between">
                <button
                  ...
                  className="text-indigo-600 flex items-center gap-3"
                >
              ...
              <div className="mt-6">
                {post.comments.map((comment) => (
                  <div
                    key={comment.id}
                    className="border p-4 mt-2 rounded space-y-5"
                  >
                    <p>{comment.text}</p>
